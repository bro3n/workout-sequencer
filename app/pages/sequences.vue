        ...
        <div class="flex justify-between items-start mb-4">
          <div class="flex-1">
            ...
            <div class="text-sm space-y-2">
              ...
              <div v-else class="flex items-center gap-2 text-gray-600 dark:text-gray-300">
                <UIcon name="i-heroicons-pause-circle" class="w-4 h-4" />
              ...
              <div class="flex items-start gap-2">
                ...
                <div class="space-y-1">
                  <p class="text-blue-800 dark:text-blue-300">
                    <span class="font-medium">{{ $t("sequences.createdAt") }}</span>
                    {{ formatDate(sequence.createdAt) }}
                  </p>
                  <p
                    v-if="sequence.updatedAt.getTime() !== sequence.createdAt.getTime()"
                    class="text-blue-800 dark:text-blue-300"
                  >
                    <span class="font-medium">{{ $t("sequences.updatedAt") }}</span>
          ...
          <div class="grid gap-2">
            ...
            >
              ...
              <span class="text-sm text-gray-600 dark:text-gray-300">
                ...
                <template v-else> {{ exercise.duration }}s </template>
              </span>
            </div>
          </div>
        </div>
      ...
      <template #footer>
        <div class="flex justify-end gap-3">
          <UButton
            ...
            @click="cancelDelete"
          />
          <UButton :label="$t('sequences.confirmDelete')" color="error" @click="confirmDelete" />
        </div>
      </template>
    </UModal>
    ...
    <UModal v-model:open="deleteAllModalOpen" :title="$t('sequences.deleteAllModalTitle')">
      <template #body>
        ...
        <p class="text-sm font-semibold text-orange-600 dark:text-orange-400 mt-3">
          ...
          {{ filterType === "workout" ? $t("sequences.sequenceType") : $t("sequences.warmupType")
      ...
      <template #footer>
        <div class="flex justify-end gap-3">
          ...
          />
          <UButton :label="$t('sequences.confirmDelete')" color="error" @click="confirmDeleteAll" />
        </div>
      </template>
    </UModal>
